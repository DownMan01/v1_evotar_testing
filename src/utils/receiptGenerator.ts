...
const successColor: [number, number, number] = [0, 128, 0];
  ...
  data.selectedCandidates.forEach((selection, index) => {
    if (index % 2 === 0) {
      ...
      doc.rect(margin, y - 4, pageWidth - 2 * margin, lineGap + 4, 'F');
    }

    doc.setFont('helvetica', 'bold');
    doc.setTextColor(...primaryColor);
    doc.text(`${selection.position}:`, margin + 5, y);
    doc.setFont('helvetica', 'normal');
    doc.setTextColor(...darkText);
    doc.text(selection.candidate, margin + 65, y);

    y += lineGap + smallSpacing;
  });
...
export const downloadReceipt = (receiptContent: string, filename: string = 'voting-receipt.pdf') => {
