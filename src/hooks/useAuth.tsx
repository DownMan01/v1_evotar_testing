    ...
    try {
      ...
      if (data.user) {
        ...
        const requires2FA =
          ...
          (profileData?.role === 'Staff' || profileData?.role === 'Administrator');
        ...
        if (requires2FA && twoFactorCode) {
          ...
          const parsedSecret = OTPAuth.Secret.fromBase32((fullProfileData?.two_factor_secret || '').toString());
          ...
          const isValidCode =
            totp.validate({ token: twoFactorCode, window: 1 }) !== null;
          const isValidBackupCode = fullProfileData?.two_factor_recovery_codes?.includes(
          ...
          if (isValidBackupCode && fullProfileData?.two_factor_recovery_codes) {
            const updatedCodes = fullProfileData.two_factor_recovery_codes.filter(
