...
export const TwoFactorVerification = ({
  ...
  return (
    <Dialog open={open} onOpenChange={handleClose} aria-label={title}>
      <DialogContent className="max-w-md p-8 bg-white rounded-2xl shadow-lg border border-gray-200">
        <DialogHeader>
        ...
        <DialogDescription className="mt-3 mb-6 text-sm text-muted-foreground leading-relaxed text-center">
          {description}
        </DialogDescription>
        ...
        {error && (
          <Alert
            variant="destructive"
            className="mb-6 flex items-center gap-2 rounded-lg border border-red-300 bg-red-50 px-4 py-3 text-red-700"
            role="alert"
          >
        ...
        <div className="flex justify-center gap-4">
          {codeDigits.map((digit, idx) => (
            <input
              ...
              } bg-gray-50 focus:outline-none focus:ring-2 transition`}
        ...
        <div className="mt-8">
          <Label
            htmlFor="backup-code"
            className="mb-2 block text-sm font-medium text-gray-700 text-center"
          >
          ...
          <Input
            ...
            maxLength={8}
            className="mx-auto max-w-xs rounded-xl border border-gray-300 bg-gray-50 px-5 py-3 text-center text-lg font-mono tracking-widest text-gray-900 placeholder:text-gray-400 focus:border-primary focus:ring-2 focus:ring-primary focus:ring-offset-0"
            spellCheck={false}
            disabled={isVerifying}
          ...
          <p
            id="backup-help"
            className="mt-2 text-center text-xs text-muted-foreground select-none"
          >
        ...
        <div className="mt-10 flex gap-4">
          <Button
            variant="outline"
            onClick={handleClose}
            className="flex-1 rounded-xl border border-gray-300 text-gray-700 hover:bg-gray-100 focus:ring-2 focus:ring-primary focus:ring-offset-0"
            disabled={isVerifying}
          >
            Cancel
          </Button>
          <Button
            ...
            className="flex-1 rounded-xl bg-primary text-white hover:bg-primary/90 focus:ring-4 focus:ring-primary/40 focus:ring-offset-0 disabled:cursor-not-allowed disabled:bg-primary/60"
            aria-live="polite"
          >
            {isVerifying && (
              <svg
