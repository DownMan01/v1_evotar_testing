            ...
            <Card key={electionId} className="overflow-hidden">
              <CardHeader className="pb-3 lg:pb-6 p-3 lg:p-6">
                ...
                <div className="flex flex-col gap-2 lg:flex-row lg:items-center lg:justify-between lg:gap-2">
                  <div className="flex items-center gap-2 min-w-0">
                    ...
                    <CardTitle className="text-base lg:text-xl truncate">{election.election_title}</CardTitle>
                  </div>
              ...
              <CardContent className="space-y-4 lg:space-y-8 p-3 lg:p-6 lg:pt-0">
                {Object.entries(election.positions)
                  .sort(([, a], [, b]) => a.rank - b.rank) // Sort by position rank (ascending)
                  .map(([positionId, position]) => (
                  <div key={positionId} className="space-y-3 lg:space-y-4">
                    {/* Mobile: Stack position title and vote count vertically */}
                    <div className="flex flex-col gap-2 lg:flex-row lg:items-center lg:justify-between lg:gap-4">
                      <h3 className="text-sm lg:text-lg font-semibold leading-tight">{position.position_title}</h3>
                    ...
                    <div className="space-y-2 lg:space-y-4">
                      {position.candidates.map((candidate, index) => {
                        ...
                        return (
                          ...
                          >
                            {/* Mobile: Stack candidate info and vote info vertically */}
                            <div className="flex flex-col gap-2 lg:flex-row lg:items-center lg:justify-between lg:gap-4 mb-2">
                              <div className="flex items-center gap-2 min-w-0">
                                {isWinner && <Trophy className="h-3 w-3 lg:h-4 lg:w-4 text-yellow-600 flex-shrink-0" />}
                                <span className="font-medium text-xs lg:text-base truncate">{candidate.candidate_name}</span>
                                {isWinner && <Badge className="bg-yellow-600 text-white text-xs">Winner</Badge>}
                              </div>
