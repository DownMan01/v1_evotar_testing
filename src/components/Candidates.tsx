...
export const Candidates = () => {
  ...
  // Sort candidates by position rank within each election (highest rank first)
  Object.keys(candidatesByElection).forEach(key => {
    candidatesByElection[key].candidates.sort((a, b) => {
      const rankA = getPositionRank(a.position_title);
      const rankB = getPositionRank(b.position_title);
      
      // Primary sort: by position rank (ascending - lower number = higher rank)
      if (rankA !== rankB) {
        return rankA - rankB;
      }
      
      // Secondary sort: by candidate name (alphabetical) if same position
                      ...
                      <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-4 lg:gap-6">
                        {electionData.candidates.map(candidate => <div key={candidate.id} className="relative group">
                            ...
                            {(profile?.role === 'Staff' || profile?.role === 'Administrator') && <div className="absolute top-2 right-2 flex gap-1 lg:opacity-0 lg:group-hover:opacity-100 transition-opacity">
                                <Button size="icon" variant="outline" className="h-9 w-9 lg:h-8 lg:w-8 bg-background/90 backdrop-blur-sm" onClick={() => setEditingCandidate(candidate)}>
