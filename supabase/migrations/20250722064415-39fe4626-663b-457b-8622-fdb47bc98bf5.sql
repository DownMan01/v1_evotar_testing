-- Fix approve_pending_action function to properly handle publish_results
...
BEGIN
  ...
  CASE v_action.action_type
    ...
    WHEN 'publish_results' THEN
      -- FIXED: Only set show_results_to_voters to TRUE, do NOT change status to Completed
      -- The election status should only be changed by the auto_update_election_statuses function when end_date is reached
      UPDATE public.elections
      SET show_results_to_voters = TRUE
...
-- Create function to toggle result visibility for admins
CREATE OR REPLACE FUNCTION public.toggle_election_results_visibility(p_election_id uuid, p_show_results boolean)
 RETURNS boolean
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
BEGIN
  -- Check if user is admin
  IF NOT public.has_role(auth.uid(), 'Administrator'::public.user_role) THEN
    RAISE EXCEPTION 'Only administrators can toggle results visibility';
  END IF;
  
  -- Update the show_results_to_voters flag
  UPDATE public.elections
  SET show_results_to_voters = p_show_results
  ...
  PERFORM public.log_audit_action(
    auth.uid(),
    'Administrator'::public.user_role,
    CASE WHEN p_show_results THEN 'show_results_to_voters' ELSE 'hide_results_from_voters' END,
    'election',
    p_election_id,
    jsonb_build_object(
      'show_results_to_voters', p_show_results
    )
  );
