-- Fix all RLS policies to use correct enum values without explicit casting
-- Since the user_role enum exists, we need to update all policies
...
FOR SELECT 
USING (public.has_role(auth.uid(), 'Staff') AND requested_by = auth.uid());

CREATE POLICY "Staff can create pending actions" 
ON public.pending_actions 
FOR INSERT 
WITH CHECK (public.has_role(auth.uid(), 'Staff') AND requested_by = auth.uid());
...
FOR SELECT 
USING (public.has_role(auth.uid(), 'Administrator') OR public.has_role(auth.uid(), 'Staff'));
